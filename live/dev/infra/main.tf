module "vpc" {
  ...
  subnets_public = var.aws_subnets_public
  subnets_private = var.aws_subnets_private
...
module "email" {
  ...
  route53_id              = "${var.enable_route53 ? module.route53[0].route53_id : var.enable_route53}"
...
module "openedx" {
  ...
  s3_storage_policy_arn = module.s3.s3_storage_policy_arn
